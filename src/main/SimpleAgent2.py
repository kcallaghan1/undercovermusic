    ...
    while i in range(len(track)) and chord < len(chords):
        ...
        if msg.time == 768:
            if chord % 2 == 0:
                ...
                track.insert(i, Message("note_on", channel=0, note=chords[chord][2] + 12, velocity=50, time=0))
                track.insert(i+1, Message("note_off", channel=0, note=chords[chord][2] + 12, velocity=50, time=384))
                track.insert(i+2, Message("note_on", channel=0, note=chords[chord][1] + 12, velocity=50, time=0))
                track.insert(i+3, Message("note_off", channel=0, note=chords[chord][1] + 12, velocity=50, time=384))
                track.insert(i+4, Message("note_off", channel=0, note=msg.note, velocity=50, time=0))
                track.remove(msg)
                chord = chord + 1
            i = i + 5
